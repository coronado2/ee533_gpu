...
def main():
    ...
    ap.add_argument("--kernel", help="kernel function name (default: first found)")
    ...
    # 3. Pick kernel
    name = args.kernel or next(iter(kernels))
    if name not in kernels:
        sys.exit(f"Kernel '{name}' not found. Available: {list(kernels)}")
    print(f"\n[3] Translating: {name}")

    # 4. Translate PTX → instructions
    insns = translate(kernels[name], name)
    print(f"  {len(insns)} instructions")

    # 5. Assemble → words → files
    words    = assemble(insns)
    mem_path = args.o + ".mem"
    lst_path = args.o + ".lst"
    write_mem(words, mem_path)
    write_lst(words, lst_path, name)

    print(f"\n[4] Output")
    print(f"  {mem_path}  ← $readmemh(\"{mem_path}\", imem) in gpu_core.v")
    print(f"  {lst_path}  ← listing\n")
    for i, w in enumerate(words):
        print(f"  [{i:03d}]  {w:08X}  {disasm(w)}")
